module collis_ions
  ...
  !
    subroutine coleff(efcolf,velrat,enrat,p,dpp,dhh,fpeff)
  !
  ...
  !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
  !
    subroutine collis_init(am0,Z0,am1,am2,Z1,Z2,densi1,densi2,tempi1,tempi2,tempe,eion, &
                           v0,nu_perp0,efcolf,velrat,enrat)
  !
  ...
  !                am1,am2       - mass numbers of the first and second background ion species
  ...
  !                eion        - initial ion energy, eV
  !   Output variables:
  !        formal: v0            - initial ion velocity, cm/s
  !        module collis_ions:
  !                efcolf - normalized collision frequencies
  !                velrat - ratio of initial ion velocity v0 to the
  !                         specific background particle thermal velocity $v_{t}=\sqrt(2T/m)$
  !                enrat  - ratio of initial ion energy to the background species
  !                         energy
  !
    double precision, dimension(nsorts) :: efcolf,velrat,enrat
    double precision :: am0,Z0,am1,am2,Z1,Z2,densi1,densi2,tempi1,tempi2,tempe,eion,dense
    double precision :: v0,vti1,vti2,vte
    double precision :: pi,pmass,emass,e,ev,alame,frecol_base,alami1,alami2
    double precision :: p,x1,x2,xe,k,nu_perp0
  !
  ...
  !
    ...
    alami1=23.d0-log(max(epsilon(1.d0), &
           sqrt(densi1*Z1**2/tempi1)*Z0*Z1*(am0+am1)/(am0*tempi1+am1*eion)))
    alami2=23.d0-log(max(epsilon(1.d0), &
           sqrt(densi2*Z2**2/tempi2)*Z0*Z2*(am0+am2)/(am0*tempi2+am2*eion)))
    alame=24.d0-log(sqrt(dense)/tempe)
    frecol_base=2.d0*pi*dense*e**4*Z0**2/((am0*pmass)**2*v0**3) !usual
  ...
  !
    x1 = am1*pmass*v0**2/(2*k*tempi1)
    x2 = am2*pmass*v0**2/(2*k*tempi2)
    xe =     emass*v0**2/(2*k*tempe)
    !since x1, x2 and xe are all much smaller than one, the approximation for fast particles can be taken in nrl page 32
    !(2018 edition) to evaluate nu_perp (do this in amor refined way, e.g. by using the exact formula on page 31 later)
    nu_perp0 = Z0**2*(Z1**2*alami1*densi1+Z2**2*alami2*densi2)*1.8d-7*am0**(-0.5)*(ev*eion)**(-1.5)
  !
    end
  !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
  !
  ...
  !
    p=z(4)
  !
    call coleff(efcolf,velrat,enrat,p,dpp,dhh,fpeff)
  !
    ierr=0
  !
