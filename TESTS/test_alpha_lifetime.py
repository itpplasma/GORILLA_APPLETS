...
# Alpha lifetime tracing. Parameters are deliberately kept small so the
# polynomial pusher stays within its stable range (larger `time_step`
# triggers platform-dependent "trouble shooting" bailouts and makes the
# per-particle `t_confined` output chaotic across compilers/LAPACK). This
# is a correctness test; perf sensitivity of the pusher is left to tests
# without a quantitative reference.
