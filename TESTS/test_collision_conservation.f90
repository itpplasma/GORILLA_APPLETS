program test_collision_conservation
    ...
    call test_signed_delta_f_ratio
...
contains
    ...
    subroutine test_signed_delta_f_ratio

        real(dp), parameter :: test_mass = 3.0_dp
        real(dp), parameter :: background_mass = 7.0_dp
        real(dp), parameter :: ratio = -0.2_dp
        real(dp), parameter :: delta_vpar = 0.35_dp
        real(dp), parameter :: delta_energy = -0.8_dp
        real(dp), parameter :: tolerance = 64.0_dp*epsilon(1.0_dp)
        real(dp) :: background_vpar
        real(dp) :: background_temperature
        real(dp) :: momentum_before, momentum_after
        ...
        background_vpar = -0.6_dp
        background_temperature = 2.7_dp
        momentum_before = background_mass*background_vpar
        energy_before = 0.5_dp*background_mass*background_vpar**2 + &
            1.5_dp*background_temperature
        ...
        if (abs(momentum_after - momentum_before) > tolerance) &
            error stop 'signed-weight momentum balance failed'
        if (abs(energy_after - energy_before) > tolerance) &
            error stop 'signed-weight energy balance failed'

    end subroutine test_signed_delta_f_ratio
