...
switch plot_quantity
    ...
    case 'vpar'
        radial_ylabel = 'Volume-averaged $\langle j_\parallel \rangle$ [statA$/$cm$^2$]';
...
figure
plot(r_values, moment_radial_avg*q, 'b-', 'LineWidth', 2)
hold on
%plot(r_values,n/3*v_th^2*tau_c/t_tr*(1-1.46*sqrt((r_values-R)/R)))
plot(r_values,c*R*n*T/A_phi_max*1.46*sqrt((r_values-R)/R)/1.5)
hold off
